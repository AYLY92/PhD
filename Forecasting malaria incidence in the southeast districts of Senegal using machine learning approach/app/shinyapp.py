...
df_malaria = load_data(file_path="D:/ufr/PhD/App/data/dfmalaria.xlsx")

# Charger le modèle
xgb_model2 = joblib.load('D:/ufr/PhD/App/model/XGBoostmodel2.pkl')

# Charger les données des régions du Sénégal
senegal_regions = load_data(file_path="D:/ufr/PhD/App/data/gadm41_SEN_1.json")
...
def plot_prediction_map_with_regions(prediction_result_df, date):
    ...
    <div style="position: fixed;
                bottom: 50px; left: 10px; width: 170px;
                background-color: white; border:2px solid grey;
                z-index:9999; font-size:14px; padding: 8px 10px;">
        <p style="font-weight: bold; text-align: center; margin: 0 0 10px 0;">Legend</p>

        <p style="margin: 10px 0; display: flex; align-items: center;">
            <span style="display:inline-block; width:20px; height:20px; background-color:#008000; border-radius:50%; margin-right:12px;"></span>
            Low incidence
        </p>

        <p style="margin: 10px 0; display: flex; align-items: center;">
            <span style="display:inline-block; width:20px; height:20px; background-color:#FFFF00; border-radius:50%; margin-right:12px;"></span>
            Medium incidence
        </p>

        <p style="margin: 10px 0; display: flex; align-items: center;">
            <span style="display:inline-block; width:20px; height:20px; background-color:#FF0000; border-radius:50%; margin-right:12px;"></span>
            High incidence
        </p>
    </div>
    '''
...
def render_folium_map(map_object):
    ...
    map_path = www_dir / map_filename

    map_object.save(str(map_path))

    return f'<iframe src="{map_filename}" width="100%" height="500" style="border:none;"></iframe>'
...
forecast_ui = ui.page_fluid(
    ...
    ui.page_sidebar(
        ui.sidebar(
            ...
            ui.input_slider("horizon", "Forecast horizon", min=1, max=12, value=6, step=1, ticks=True, sep=''),
...
app_ui = ui.page_navbar(
   ...
   ui.nav_control(
    ui.tags.a(
        "SOURCE CODE",
        id="source-code-link",
        href="https://github.com/AYLY92/PhD",
        target="_blank",
        class_="source-code-tab",
    )
),
    title=ui.tags.div(
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (sourceCodeLink) {
            sourceCodeLink.addEventListener('click', function() {
                sourceCodeLink.classList.add('active-source');
            });
        }
        ...
        document.querySelectorAll('.navbar .nav-link').forEach(tab => {
            tab.addEventListener('click', function() {
                if (sourceCodeLink) {
                    sourceCodeLink.classList.remove('active-source');
                }
            });
        });
    });
...
def server(input, output, session):
    ...
    def download_csv():
        ...
        yield predictions_df().to_csv(index=False, encoding="utf-8-sig")
